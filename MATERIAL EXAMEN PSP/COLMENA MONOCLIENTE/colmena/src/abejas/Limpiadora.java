...
public class Limpiadora extends Thread {
    ...
    private int intentosSolicitud = 0;
    ...
    public void run() {
        try {
            ...
            while (!Thread.currentThread().isInterrupted() && conectado) {
                boolean zonaAsignada = solicitarZona();

                // Si no pudo obtener una zona después de varios intentos, esperar un tiempo antes de volver a intentar
                if (!zonaAsignada) {
                    intentosSolicitud++;
                    if (intentosSolicitud >= 3) {
                        ColorConsole.printLimpiadora("Limpiadora-" + id + ": Demasiados intentos fallidos. Esperando un tiempo...");
                        Thread.sleep(10000); // Esperar 10 segundos
                        intentosSolicitud = 0;
                    } else {
                        ColorConsole.printLimpiadora("Limpiadora-" + id + ": Esperando un momento antes de volver a solicitar zona...");
    ...
    private boolean solicitarZona() {
        try {
            ...
            ColorConsole.printLimpiadora("Limpiadora-" + id + ": Solicitando zona a la reina...");
            out.writeUTF("SOLICITAR_ZONA");
            String zona = in.readUTF();

            // Verificar si la respuesta indica un error
            if (zona.startsWith("Solo las limpiadoras") || zona.contains("error")) {
                ColorConsole.printLimpiadora("Limpiadora-" + id + ": Error al solicitar zona: " + zona);
                return false;
            }
