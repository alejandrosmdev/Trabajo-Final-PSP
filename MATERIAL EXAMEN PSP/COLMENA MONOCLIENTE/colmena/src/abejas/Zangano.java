...
public class Zangano extends Thread {
    ...
    private int intentosAlimentacion = 0;
        ...
        try {
            ...
            while (!Thread.currentThread().isInterrupted() && conectado) {
                ...
                if (random.nextInt(10) == 0) { // 1 en 10 posibilidades
                    ...
                    boolean alimentado = alimentarse();

                    // Si no pudo alimentarse después de varios intentos, esperar un tiempo antes de volver a intentar
                    if (!alimentado) {
                        intentosAlimentacion++;
                        if (intentosAlimentacion >= 3) {
                            ColorConsole.printZangano("Zángano-" + id + ": Demasiados intentos fallidos de alimentación. Esperando un tiempo...");
                            Thread.sleep(10000); // Esperar 10 segundos
                            intentosAlimentacion = 0;
                        } else {
                            ColorConsole.printZangano("Zángano-" + id + ": Esperando un momento antes de volver a intentar alimentarse...");
    ...
    private boolean alimentarse() {
        try {
            ...
            // Verificar si la respuesta indica que no hay nodrizas disponibles
            if (respuesta.contains("No hay nodrizas disponibles")) {
                return false;
            }
