...
public class ServidorColmena {
    ...
    private ServidorColmena() {
        try {
            ...
            ColorConsole.printInfo("Servidor Colmena iniciado en el puerto: " + PUERTO);
        ...
        new Thread(() -> {
            while (running) {
                ...
                try {
                    ...
                    ColorConsole.printInfo("Nueva conexión aceptada desde: " + clienteSocket.getInetAddress().getHostAddress());
                    ...
                    if (running) {
                        ColorConsole.printInfo("Error en el socket al aceptar conexión: " + se.getMessage());
                    }
                } catch (IOException e) {
                    // No mostrar error si el servidor está cerrándose
                    if (running && e.getMessage() != null && !e.getMessage().equals("null")) {
                        ColorConsole.printInfo("Error al aceptar conexión: " + e.getMessage());
                    }
                } catch (Exception e) {
                    // No mostrar error si el mensaje es null
                    if (running && e.getMessage() != null && !e.getMessage().equals("null")) {
                        ColorConsole.printInfo("Error inesperado al aceptar conexión: " + e.getMessage());
                    }
                }
            }
        }).start();
    ...
    public synchronized String asignarZona() {
        reinaOcupada = true;
        ColorConsole.printInfo("Reina: Asignando zona...");
        ...
        ColorConsole.printInfo("Reina: Zona asignada: " + zona);
        reinaOcupada = false;
        return zona;
    }
    ...
    public synchronized int alimentarZangano(int idNodriza, String zangano) {
        ...
        ColorConsole.printInfo("Nodriza-" + idNodriza + ": Alimentando a " + zangano);
        ...
        ColorConsole.printInfo("Nodriza-" + idNodriza + ": " + zangano + " alimentado");
    ...
    public void cerrar() {
        ...
        try {
            if (serverSocket != null && !serverSocket.isClosed()) {
                serverSocket.close();
                ColorConsole.printInfo("Servidor Colmena cerrado.");
            }
        } catch (IOException e) {
            ColorConsole.printInfo("Error al cerrar el servidor: " + e.getMessage());
        }
    }
}